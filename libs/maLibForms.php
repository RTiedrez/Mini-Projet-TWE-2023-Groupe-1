...
// Vraiment pas élégant
function mkInput($type,$name,$value="",$misc=[]) //$misc : tab asso cles: id, label, positionLabel
{
	// Produit un champ formulaire, avec label si $misc est défini
	if (array_key_exists("id", $misc)) {
		if (array_key_exists("label", $misc)) {
			if (array_key_exists("positionLabel", $misc)) {
				if ($misc["positionLabel"] != "apres") {
					echo "<label for=\"$misc[id]\">$misc[label]</label>";
					echo "<input type=\"$type\" name=\"$name\" value=\"$value\" id=\"$misc[id]\"/>\n";
